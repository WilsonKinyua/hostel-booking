    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.notices.update", [$notice->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.notice.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $notice->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notice') ? 'is-invalid' : '' }}" name="notice" id="notice">{!! old('notice', $notice->notice) !!}</textarea>
